...
test("frontend uses the same-origin marketplace results API before fallbacks", async () => {
  ...
  assert.match(client, /transport: "frontend-api"/);
...
test("marketplace API is a bounded raw relay without Browser Run or parsing loops", async () => {
  ...
  assert.match(route, /MAX_BODY_BYTES = 2_000_000/);
  assert.match(route, /UPSTREAM_TIMEOUT_MS = 10_000/);
  assert.match(route, /transport: "frontend-api"/);
...
test("health and deployment identify frontend API relay revision", async () => {
  ...
  assert.match(health, /frontend-marketplace-results-api-v9/);
  assert.match(wrangler, /frontend-api-relay/);
