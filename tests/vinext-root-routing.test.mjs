...
const wrangler = await readFile(new URL("../wrangler.vinext-build.toml", import.meta.url), "utf8");
...
test("Vinext full-stack development uses the exported App Router Worker entry", () => {
  ...
  assert.match(wrangler, /^binding\s*=\s*"ASSETS"/m);
...
test("the root App Router page and Vinext plugin remain registered", () => {
  ...
  assert.match(vite, /configPath:\s*["']\.\/wrangler\.vinext-build\.toml["']/);
});
